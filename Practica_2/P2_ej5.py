...
### Forward
def forward(x,W1,b1,W2,b2,RELU=False,sigm=False):
    # x: (N,3072)
...
def loss_binary_cross_entropy(y_pred, y_true):
    eps = 1e-10
...
### Backpropagation
def backward(y_pred, y_true, val_med, W1, W2, reg_lambda, MSE, CCE, RELU, sigm):
    x, z1, a1, z2 = val_med
    N = y_true.shape[0]

    if MSE:
        if sigm:
            dz2 = (y_pred - y_true) * derivada_sigmoid(y_pred) / N
        elif RELU: 
            dz2 = (y_pred - y_true) * derivada_ReLU(y_pred) / N
        else:
            dz2 = (y_pred - y_true) / N                 # (N, output)
    if CCE:
        p = softmax(y_pred)                        # aplicar softmax
        dz2 = (p - y_true) / N                     # derivada de CCE
        #dz2 = (y_pred - y_true) / N                     # derivada de CCE
    ...
    dz1 = da1*derivada_ReLU(a1)              # (N, hidden)
...
def train(x_train,y_train,x_test,y_test,
            epochs=25,batch_size=200,lr=0.01,lreg=1e-4,print_cada=1, 
            MSE=False, CCE=False, RELU=False, sigm=False):
    ...
    W1,b1,W2,b2=params_init(input_dim,hidden_dim,output_dim,seed=123)
    ...
    for epoch in range(1,epochs+1):
        ...
        for i in range(num_batches):
            ...
            y_pred, val_med = forward(xb, W1, b1, W2, b2, RELU, sigm)     # FORWARD
            dW1,db1,dW2,db2 = backward(y_pred, yb, val_med, W1, W2, lreg, MSE, CCE, RELU, sigm)   # BACKWARD
        ...
        if epoch % print_cada == 0:
            print(f"Epoch {epoch}/{epochs}  train_loss={train_loss:.4f}  test_loss={test_loss:.4f}  train_acc={train_acc:.4f}  test_acc={test_acc:.4f}")
    params = (W1, b1, W2, b2)
