...
def loss(y_pred, y_true, W1, W2, lreg):
...
### Backpropagation
def backward(y_pred, y_true, val_med, W1, W2, reg_lambda):
    x, z1, a1, z2 = val_med
    N = y_true.shape[0]

    # dL/dz2 : para MSE L = 1/(2N) * sum ||z2 - y||^2 -> derivada = (1/N) * (z2 - y)
...
def train(x_train,y_train,x_val,y_val,
    ...
    for epoch in range(1,epochs+1):
        ...
        for i in range(num_batches):
            ...
            loss_batch = loss(y_pred, yb, W1, W2, lreg)     # LOSS
            epoch_loss += loss_batch*(end-start)    # sumo loss pesada por batch_size

            dW1,db1,dW2,db2 = backward(y_pred, yb, val_med, W1, W2, lreg)   # BACKWARD
        ...
        if epoch % print_cada == 0:
            print(f"Epoch {epoch}/{epochs}  train_loss={train_loss:.4f}  test_loss={val_loss:.4f}  train_acc={train_acc:.4f}  test_acc={val_acc:.4f}")
